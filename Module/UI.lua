-- Mercury Reworked by @gxs9, based on the original framework by Abstract & Deity.
...
local Library = {
	Themes = {
		Dark = {
			...
			Tertiary = Color3.fromRGB(255, 125, 0),
		...
		Ultraviolet = {
			...
			Tertiary = Color3.fromRGB(127,0,255),
			StrongText = Color3.fromRGB(255,255,255),
			WeakText = Color3.fromRGB(255,255,255)
		},
		Emerald = {
			...
			Tertiary = Color3.fromRGB(0, 255, 128),
			StrongText = Color3.fromRGB(255, 255, 255),
			WeakText = Color3.fromRGB(150, 255, 200)
		},		
		Ikea = {
			Main = Color3.fromRGB(57,57,67),
			Secondary = Color3.fromRGB(0,120,215),
			Tertiary = Color3.fromRGB(243,181,82),
			StrongText = Color3.fromRGB(255,255,255),
			WeakText = Color3.fromRGB(255,255,255)
		},
		Abyss = {
			...
			Tertiary = Color3.fromRGB(0,120,215),
			StrongText = Color3.fromRGB(255,255,255),
			WeakText = Color3.fromRGB(255,255,255)
		},
	},
...
function Library:create(options)
	...
	local settings = {Theme = "nil"}
	if readfile and writefile and isfile then
		if not isfile("EGlob.json") then
			writefile("EGlob.json", HTTPService:JSONEncode(settings))
		end
		settings = HTTPService:JSONDecode(readfile("EGlob.json"))
	...
	if getgenv and getgenv().EternalUI then
		getgenv():EternalUI()
		getgenv().EternalUI = nil
	end
	...
	local closeButton = core:object("ImageButton", {
		...
		Image = "http://www.roblox.com/asset/?id=10747383819",
		AnchorPoint = Vector2.new(1)
	})

	closeButton.MouseEnter:connect(function()
		closeButton:tween{ImageColor3 = Color3.fromRGB(175, 0, 0)}
	end)
	...
	local searchIcon = urlBar:object("ImageLabel", {
		...
		Image = "http://www.roblox.com/asset/?id=10734943674",
		BackgroundTransparency = 1
	})
	...
	local homeButtonIcon = homeButton:object("ImageLabel", {
		...
		Image = "http://www.roblox.com/asset/?id=10723407389",
		Theme = {ImageColor3 = "StrongText"}
	})
	...
	local displayName; do
		...
		local displayName = profile:object("TextLabel", {
			RichText = true,
			Text = "Welcome Back, <font color='rgb(" ..  math.floor(c.R*255) .. "," .. math.floor(c.G*255) .. "," .. math.floor(c.B*255) .. ")'> <b>" .. LocalPlayer.DisplayName .. "</b> </font>",
			TextScaled = true,
	...
	do
		local desiredInterval = 1 -- Update every second
		local counter = 0
		...
		RunService.Heartbeat:Connect(function(step)
			...
			if counter >= desiredInterval then
				...
				-- Correctly determine the suffix
				local suffix
				if day % 100 >= 11 and day % 100 <= 13 then
					suffix = "th" -- Handle 11, 12, 13 as special cases
				else
				...
				timeDisplay.Text = string.format("%s, %d%s %s %s", month, day, suffix, year, time)
			end
		end)
	end	
	...
	do -- Settings
		...
		settingsTab:Dropdown{
			...
			StartingText = nil,
			Description = "Changes The Sound Of Notification Pings",
			Items = {
			{'Default Ping', 578970639},
			{'Fast Ping', 2668759868},
			{'Dull Ping', 17208372272},
			{'Futuristic Ping', 8747340426},
			{'Roblox+ Ping', 232127604},
		},
			Callback = function(S)
			Library.NotificationSound = S
			end
		}
		settingsTab:slider{
			Name = "Drag Speed",
			Description = "How smooth the UI drag is.",
			Increment = 0.1,
			Max = 20,
			Default = 0,
        ...
        do--//Test//
            local tests=settingsTab:Section{Name="Mobile Toggle Button"}
            do
                local MBT = {[2] = 25}
                ...
                tests:toggle{Name = "Enable Button", Description = "Peak Mobile Support.", StartingState = false, Callback = function(state) MBT[1] = state end}
                tests:Slider{Name = "Button Size", Default = 25, Increment = 1, Min = 1, Max = 50, Callback = function(V) MBT[2] = V end}
            end
        end
	end
...
function Library:notification(options)
    options = self:set_defaults({
        ...
        Ping = true  -- Updated variable name
    }, options)
    ...
    if options.Ping then
        ...
        sound.SoundId = "rbxassetid://" .. Library.NotificationSound
        sound.Volume = 1
        sound.Looped = false
...
function Library:slider(options)
    ...
    -- Function to round value to the nearest increment based on the number of decimals
    local function roundToIncrement(value, increment)
        local decimals = 0
        if increment % 1 ~= 0 then
            local decimalPart = string.match(tostring(increment), "^(.-)%.") or "0"
    ...
    do
        ...
        sliderContainer.MouseButton1Down:connect(function()
            ...
            while RunService.RenderStepped:wait() and down do
                ...
                -- Adjust value by Increment
                value = roundToIncrement(value, options.Increment)
                
                -- Update the valueText with the rounded value
